classdef EdgeEle_linear < handle
    ...
    properties
        ...
        % 4*n array where n is total element number. (global element,
        % local node) -> global node. n-th column is n-th element [lu;ru;ld;rd]
        el2no;
        
        % information of every edge: [start pt, end pt]
        edge_info;
        
        % 4*n array where n is total element number. (local edge, global
        % element) -> global edge number [lu;ru;ld;rd]
        el2ed;
    ...
    methods
        function obj = EdgeEle_linear(N)
            ...
            obj.at = 0; obj.bt = 1; % t direction
            obj.e0 = 1; obj.mu0 = 1;
            obj.ge = 1; obj.we = 1;
            obj.gm = 1; obj.wm = 1;
            obj.npt = obj.Nx*obj.Ny;
            obj.nele = (obj.Nx-1)*(obj.Ny-1);
            obj.nedg = obj.Nx*(obj.Ny-1) + obj.Ny*(obj.Nx-1);
...
%             obj.g1_1 = @(x,y,t)0;
%             obj.g1_2 = @(x,y,t)0;
%             obj.g2 = @(x,y,t)(1-2*x).*y.*(1-y);
%             obj.g3_1 = @(x,y,t)0;
%             obj.g3_2 = @(x,y,t)-x.*(1-x).*y.*(1-y).*obj.we^2;
%             obj.g4 = @(x,y,t)0;
            ...
            obj.edge_info = zeros(obj.nele,2);
...
%         Form_Bound(obj);    % form boundary contribution
        ...
        Plot_Sol(obj);
        %% tool methods
        y = Basis_fcn1(x,y,nn,ne,obj);    % (x, # of element, # of edge)
    end
